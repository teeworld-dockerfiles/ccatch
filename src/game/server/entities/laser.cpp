...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	else if(m_Type == WEAPON_LASER)
	{
		if(pHit->GetPlayer()->m_TeeInfos.m_ColorBody != pOwnerChar->GetPlayer()->m_TeeInfos.m_ColorBody)
		{
			...
			msg.m_Victim = pHit->GetPlayer()->GetCID();
			msg.m_Weapon = WEAPON_LASER;
			Server()->SendPackMsg(&msg, MSGFLAG_VITAL, -1);
			GameServer()->CreateSoundGlobal(SOUND_HIT, m_Owner);
			GameServer()->CreateDeath(pHit->m_Pos, -1, CmaskAll());
			GameServer()->CreateSoundGlobal(SOUND_PICKUP_HEALTH, pHit->GetPlayer()->GetCID());
		}
	}
	return true;
}
...
void CLaser::DoBounce()
{
	...
	if(Res)
	{
		if(!HitCharacter(m_Pos, To))
		{
			...
			CGameControllerCcatch *pControllerDDRace = (CGameControllerCcatch *)GameServer()->m_pController;
