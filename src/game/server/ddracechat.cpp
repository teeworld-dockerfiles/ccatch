...
#include <game/server/gamemodes/ccatch.h>
...
void CGameContext::ConPractice(IConsole::IResult *pResult, void *pUserData)
{
	...
	CGameTeams &Teams = ((CGameControllerCcatch *)pSelf->m_pController)->m_Teams;
...
void CGameContext::ConLockTeam(IConsole::IResult *pResult, void *pUserData)
{
	...
	int Team = ((CGameControllerCcatch *)pSelf->m_pController)->m_Teams.m_Core.Team(pResult->m_ClientID);

	bool Lock = ((CGameControllerCcatch *)pSelf->m_pController)->m_Teams.TeamLocked(Team);
	...
	{
		((CGameControllerCcatch *)pSelf->m_pController)->m_Teams.SetTeamLock(Team, true);
...
{
	((CGameControllerCcatch *)m_pController)->m_Teams.SetTeamLock(Team, false);
...
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	CGameControllerCcatch *pController = (CGameControllerCcatch *)pSelf->m_pController;
	const char *pName = pResult->GetString(0);
	...
	else
	{
		...
		else
		{
			str_format(
				...
				sizeof(aBuf),
				"You are in team %d",
				((CGameControllerCcatch *)pSelf->m_pController)->m_Teams.m_Core.Team(pResult->m_ClientID));
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "chatresp",
				aBuf);
		}
	}
