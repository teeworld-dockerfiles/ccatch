...
void CGameContext::SendChatTeam(int Team, const char *pText)
{
	for(int i = 0; i < MAX_CLIENTS; i++)
		if(((CGameControllerCcatch *)m_pController)->m_Teams.m_Core.Team(i) == Team)
...
void CGameContext::SendChat(int ChatterClientID, int Team, const char *pText, int SpamProtectionClientID, int Flags)
{
	...
	{
		CTeamsCore *pTeams = &((CGameControllerCcatch *)m_pController)->m_Teams.m_Core;
		CNetMsg_Sv_Chat Msg;
		Msg.m_Team = 1;
		Msg.m_ClientID = ChatterClientID;
		Msg.m_pMessage = aText;
...
void CGameContext::OnPreTickTeehistorian()
{
	...
	auto *pController = ((CGameControllerCcatch *)m_pController);
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
...
bool CGameContext::OnClientDDNetVersionKnown(int ClientID)
{
	...
	((CGameControllerCcatch *)m_pController)->m_Teams.SendTeamsState(ClientID);
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			int GameTeam = ((CGameControllerCcatch *)m_pController)->m_Teams.m_Core.Team(pPlayer->GetCID());
			if(Team)
...
//			}
	...
	if(MsgID == NETMSGTYPE_CL_STARTINFO)
	{
		...
		pPlayer->m_TeeInfos.m_UseCustomColor = 1;
//		pPlayer->m_TeeInfos.m_ColorBody = pMsg->m_ColorBody;
		pPlayer->m_TeeInfos.m_ColorFeet = ColorHSLA(0, 0, 1).Pack();
		if(!Server()->IsSixup(ClientID))
			pPlayer->m_TeeInfos.ToSixup();
...
void CGameContext::OnInit()
{
	...
	m_pController = new CGameControllerCcatch(this);
...
{
	CGameControllerCcatch *pController = (CGameControllerCcatch *)m_pController;
...
bool CGameContext::IsDefaultSkin(const char *skinName)
{
	char const skins[16][64] = {
		"default",
		"bluekitty",
		"twinbop",
		"bluestripe",
